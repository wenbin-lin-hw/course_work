"""
TabNet Model for Predicting Appendicitis Severity with Over-sampling and Grid Search
This script trains a TabNet model to predict the 'Severity' column
...
def train_tabnet_with_gridsearch(X_train, y_train, X_test, y_test):
    """
    Train TabNet model with Grid Search for hyperparameter optimization
    ...
    Returns:
    --------
    best_model : trained TabNetClassifier with best parameters
    best_params : dict of best parameters
    grid_search_results : list of all results
    """
    print("\n" + "="*60)
    print("Training TabNet model with Grid Search...")
    print("="*60)
    ...
    print("Note: This may take a while. Training with reduced epochs for grid search.\n")
                    ...
                    for lambda_sparse in param_grid['lambda_sparse']:
                        for learning_rate in param_grid['learning_rate']:
                            ...
                            try:
                                ...
                                # Train the model (reduced epochs for grid search)
                                model.fit(
    ...
    for param, value in best_params.items():
        print(f"  {param}: {value}")
    print(f"\nBest test accuracy: {best_score:.4f}")
    ...
    return final_model, best_params, grid_search_results
...
def evaluate_model(model, X_train, X_test, y_train, y_test):
    ...
    plt.title('Confusion Matrix - TabNet with Grid Search (with Over-sampling)')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.tight_layout()
    plt.savefig('confusion_matrix_tabnet_gridsearch_oversampling.png', dpi=300, bbox_inches='tight')
    print("\nConfusion matrix saved as 'confusion_matrix_tabnet_gridsearch_oversampling.png'")
    plt.close()
    
    # ROC curve and AUC (for binary classification)
    if len(np.unique(y_test_np)) == 2:
        y_test_proba = model.predict_proba(X_test_np)[:, 1]
        ...
        plt.title('ROC Curve - TabNet with Grid Search (with Over-sampling)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('../../results/roc_curve_tabnet_gridsearch_oversampling.png', dpi=300, bbox_inches='tight')
        print("ROC curve saved as 'roc_curve_tabnet_gridsearch_oversampling.png'")
        plt.close()
...
def plot_class_distribution(y_original, y_resampled, y_test):
    ...
    plt.tight_layout()
    plt.savefig('../../output/class_distribution_tabnet_gridsearch_oversampling.png', dpi=300, bbox_inches='tight')
    print("Class distribution plot saved as 'class_distribution_tabnet_gridsearch_oversampling.png'")
    plt.close()
...
def plot_feature_importance(model, feature_names, top_n=20):
    ...
    plt.title(f'Top {top_n} Feature Importances - TabNet with Grid Search (with Over-sampling)')
    plt.tight_layout()
    plt.savefig('../../output/feature_importance_tabnet_gridsearch_oversampling.png', dpi=300, bbox_inches='tight')
    print(f"\nFeature importance plot saved as 'feature_importance_tabnet_gridsearch_oversampling.png'")
    plt.close()
...
def plot_grid_search_results(grid_search_results):
    ...
    results_df = pd.DataFrame([
        {**result['params'], 'test_accuracy': result['test_accuracy']}
    ...
    params_to_plot = ['n_d', 'n_a', 'n_steps', 'gamma', 'lambda_sparse', 'learning_rate']
    
    for idx, param in enumerate(params_to_plot):
        ax = axes[idx // 3, idx % 3]
        
        # Group by parameter and calculate mean accuracy
        grouped = results_df.groupby(param)['test_accuracy'].agg(['mean', 'std', 'count'])
        
        ax.errorbar(grouped.index, grouped['mean'], yerr=grouped['std'], 
                   marker='o', linestyle='-', linewidth=2, markersize=8, capsize=5)
        ax.set_xlabel(param)
        ax.set_ylabel('Test Accuracy')
        ax.set_title(f'Effect of {param} on Test Accuracy')
        ax.grid(True, alpha=0.3)
        ax.set_ylim([results_df['test_accuracy'].min() - 0.05, 
                     results_df['test_accuracy'].max() + 0.05])
    
    plt.tight_layout()
    plt.savefig('../../results/grid_search_results_tabnet_oversampling.png', dpi=300, bbox_inches='tight')
    print("\nGrid search results plot saved as 'grid_search_results_tabnet_oversampling.png'")
    plt.close()
    
    return results_df


def compare_methods(X_train, X_test, y_train, y_test, final_feature_names, best_params):
    ...
    for method in methods:
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(y_test_np, y_test_pred, average='weighted')
    ...
    plt.tight_layout()
    plt.savefig('../../results/oversampling_methods_comparison_tabnet_gridsearch.png', dpi=300, bbox_inches='tight')
    print("\nComparison plot saved as 'oversampling_methods_comparison_tabnet_gridsearch.png'")
    plt.close()
...
def main():
    ...
    print("="*60)
    print("TabNet for Appendicitis Severity Prediction")
    print("WITH GRID SEARCH AND OVER-SAMPLING FOR CLASS BALANCE")
    print("="*60)
    ...
    model, best_params, grid_search_results = train_tabnet_with_gridsearch(
        X_train_resampled, y_train_resampled, X_test, y_test
    )
    
    # Plot grid search results
    results_df = plot_grid_search_results(grid_search_results)
    ...
    print("="*60)
    comparison_results = compare_methods(X_train, X_test, y_train, y_test, final_feature_names, best_params)
    print("\nComparison Results:")
    ...
    # model.save_model('tabnet_model_gridsearch_oversampling')
    # print("Model saved as 'tabnet_model_gridsearch_oversampling.zip'")
    #
    # # Save best parameters
    # with open('best_params_tabnet_gridsearch_oversampling.pkl', 'wb') as f:
    #     pickle.dump(best_params, f)
    # print("Best parameters saved as 'best_params_tabnet_gridsearch_oversampling.pkl'")
    #
    # # Save feature names
    # with open('feature_names_tabnet_gridsearch_oversampling.pkl', 'wb') as f:
    #     pickle.dump(final_feature_names, f)
    # print("Feature names saved as 'feature_names_tabnet_gridsearch_oversampling.pkl'")
    #
    # # Save grid search results
    # results_df.to_csv('grid_search_results_tabnet_oversampling.csv', index=False)
    # print("Grid search results saved as 'grid_search_results_tabnet_oversampling.csv'")
    #
    # print("\n" + "="*60)
