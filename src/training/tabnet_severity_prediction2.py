...
class TabNetSeverityTrainer:
    def __init__(self, data_path):
        """Initialize TabNet Severity Trainer"""
    ...
    def load_and_prepare_data(self):
        ...
        print("=" * 80)
        print("LOADING APPENDICITIS DATA FOR Severity PREDICTION")
        print("=" * 80)
    ...
    def create_tabnet_model(self, input_dim, output_dim):
        ...
        # TabNet hyperparameters optimized for Severity prediction
        tabnet_params = {
    ...
    def create_visualizations(self):
        ...
        fig.suptitle('TabNet Severity Prediction Results', fontsize=16, fontweight='bold')
...
"""
        ...
        # Save visualization
        filename = '../../results/tabnet_severity_results.png'
    ...
    def save_results(self):
        ...
        summary_df.to_csv('../../output/tabnet_severity_summary.csv', index=False)
        ...
        predictions_df.to_csv('../../output/tabnet_severity_predictions.csv', index=False)

        # Save confusion matrix
        cm_df = pd.DataFrame(self.results['confusion_matrix'],
                             index=self.results['target_names'],
                             columns=self.results['target_names'])
        cm_df.to_csv('../../output/tabnet_severity_confusion_matrix.csv')

        print(f"✓ Results saved:")
        print(f"  - tabnet_severity_summary.csv")
        print(f"  - tabnet_severity_predictions.csv")
        print(f"  - tabnet_severity_confusion_matrix.csv")
        print(f"  - tabnet_severity_results.png")

    def run_complete_training(self):
        """Run the complete training pipeline"""
        print("Starting Complete TabNet Severity Training Pipeline")
        print("=" * 70)
...
def main():
    ...
    print("TabNet Severity Prediction Training")
    print("=" * 50)
    print("Configuration:")
    print("- Target: Severity")
    print("- Excluded: US_Performed, US_Number, Management, Severity, Severity")
    print("- Sampling: Stratified")
    print("- Train:Test Ratio: 2:1")
    print()

    # Create trainer
    trainer = TabNetSeverityTrainer('../../data/appendicitis/processed_appendicitis_data_final.xlsx')

    # Run training
    success = trainer.run_complete_training()

    if success:
        print("\n✓ TabNet Severity Training completed successfully!")
    else:
        print("\n✗ TabNet Severity Training failed!")
