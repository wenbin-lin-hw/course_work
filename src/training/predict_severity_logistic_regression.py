...
def plot_feature_coefficients(model, feature_names, top_n=20):
    ...
    if hasattr(model, 'coef_'):
        ...
        plt.title(
            f'Top {top_n} Feature Coefficients - Logistic Regression (with Over-sampling)\n(Green=Positive, Red=Negative)')
        plt.axvline(x=0, color='black', linestyle='--', linewidth=1)
        plt.tight_layout()
        plt.savefig('../../output/feature_coefficients_logistic_regression_oversampling.png', dpi=300,
                    bbox_inches='tight')
...
def plot_probability_distribution(model, X_test, y_test):
    ...
    if y_proba.shape[1] == 2:
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('../../output/probability_distribution_logistic_regression_oversampling.png', dpi=300,
                    bbox_inches='tight')
