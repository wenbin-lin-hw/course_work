"""
Gaussian Naive Bayes Model for Predicting Appendicitis Severity with Over-sampling
This script trains a Gaussian Naive Bayes model to predict the 'Severity' column
...
def train_gaussian_nb(X_train, y_train, perform_cv=True):
    """
    ...
    cv_scores : cross-validation scores (if perform_cv=True)
    """
    print("\n" + "="*60)
    print("Training Gaussian Naive Bayes model...")
    print("\nAbout Gaussian Naive Bayes:")
    print("  - Assumes features follow a Gaussian (normal) distribution")
    ...
    print("  - No hyperparameters to tune (simple model)")
...
def evaluate_model(model, X_train, X_test, y_train, y_test):
    ...
    plt.title('Confusion Matrix - Gaussian Naive Bayes (with Over-sampling)')
    plt.ylabel('Actual')
    ...
    if len(np.unique(y_test)) == 2:
        ...
        plt.title('ROC Curve - Gaussian Naive Bayes (with Over-sampling)')
        plt.legend()
...
def plot_feature_statistics(model, X_train, y_train, feature_names, top_n=20):
    """
    Plot feature statistics from Gaussian Naive Bayes
    Shows mean and variance for each class
    """
    print("\n" + "="*60)
    print("Feature Statistics (Mean and Variance per Class)")
    print("="*60)
    ...
    # Features with high variance difference between classes are more important
    theta = model.theta_  # Mean of each feature per class
    sigma = model.var_    # Variance of each feature per class
    
    # Calculate variance of means across classes as importance measure
    feature_importance = np.var(theta, axis=0)
    ...
    print(f"\nTop {top_n} Most Important Features (based on variance of means across classes):")
    ...
    for idx, (feat_idx, feat_name) in enumerate(zip(top_5_indices, top_5_features)):
        if idx < 5:
            means = theta[:, feat_idx]
            stds = np.sqrt(sigma[:, feat_idx])
            
            axes[idx].bar(classes, means, yerr=stds, capsize=5, color='skyblue', edgecolor='black')
...
def compare_methods(X_train, X_test, y_train, y_test):
    ...
    for method in methods:
        ...
        # Train model
        model, _ = train_gaussian_nb(X_train_resampled, y_train_resampled, perform_cv=False)
...
def plot_probability_distributions(model, X_test, y_test):
    ...
    if len(model.classes_) == 2:
        ...
        for class_idx, class_label in enumerate(model.classes_):
            ...
            plt.hist(y_proba[mask, 1], bins=30, alpha=0.6, label=f'True Class {class_label}')
        
        plt.xlabel('Predicted Probability (Class 1)')
        plt.ylabel('Frequency')
        plt.title('Distribution of Predicted Probabilities - Gaussian NB')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
...
def main():
    ...
    print("="*60)
    print("Gaussian Naive Bayes for Appendicitis Severity Prediction")
    ...
    model, cv_scores = train_gaussian_nb(
        X_train_resampled, y_train_resampled, perform_cv=True
    )
    ...
    # Plot feature statistics and importance
    feature_importance = plot_feature_statistics(
        model, X_train_resampled, y_train_resampled, final_feature_names, top_n=20
    )
    
    # Plot probability distributions
    plot_probability_distributions(model, X_test, y_test)
    
    # Compare different over-sampling methods
    print("\n" + "="*60)
    print("BONUS: Comparing All Over-sampling Methods")
    print("="*60)
    comparison_results = compare_methods(X_train, X_test, y_train, y_test)
    print("\nComparison Results:")
